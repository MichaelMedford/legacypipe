...
#>>>>>>> 457b44ac8814215e607b45f106870ecc6666d55d
        ...
        for name,newsrc in trymodels:
            ...
            if name == 'psfexp' and newsrc is None:
                ...
                #print(src)
                newsrc = psfexp = PSFandExpGalaxy_diffcentres(
                    src.getPosition(), src.getBrightness()*0.5, exp.getShape(),
                    src.getPosition(),src.getBrightness()*0.5).copy()
            ...
            if isinstance(newsrc, (DevGalaxy, ExpGalaxy)):
                ...
                oldshape = (newsrc.shapeExp, newsrc.shapeDev,newsrc.fracDev)
            elif isinstance(newsrc, PSFandExpGalaxy_diffcentres):
                oldshape = (newsrc.shapeExp)
            elif isinstance(newsrc, PSFandDevGalaxy_diffcentres):
                oldshape = (newsrc.shapeDev)
            elif isinstance(newsrc, PSFandCompGalaxy_diffcentres):
                oldshape = (newsrc.shapeExp, newsrc.shapeDev)
        ...
        if self.plots_per_source:
            ...
            plt.clf()
            rows,cols = 3, 6
            modnames = ['none', 'ptsrc', simname, 'dev', 'exp', 'comp', 'psfexp', 'psfdev', 'psfcomp']
...
def _select_model(chisqs, nparams, galaxy_margin, galaxypsf_margin, comppsf_margin, rex):
    ...
    #    return keepmod
    if 'comp' in chisqs:
        diff = chisqs.get('comp',0) - chisqs[keepmod]
        if diff > cut:
            keepmod = 'comp'
    ...
    exppsfdiff = chisqs.get('psfexp',0) - chisqs[keepmod]
    devpsfdiff = chisqs.get('psfdev',0) - chisqs[keepmod]
    comppsfdiff = chisqs.get('psfcomp',0) - chisqs[keepmod]  
